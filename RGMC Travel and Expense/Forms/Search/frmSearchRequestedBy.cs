...
namespace MyRIS
{
    public partial class frmSearchRequestedBy : Form
    {
        private Function function = new Function();

        private long mvEPVId = -1;
        ...
        private void LoadListView()
        {
            ...
            try
            {
                sSQL.AppendLine("SELECT employeeId, CONCAT(lastName,', ', firstName) AS [Employee Name]");
                sSQL.AppendLine("FROM Employee");
                ...
                if (strName != "")
                { sSQL.AppendLine("    AND lasName LIKE " + (strName + "%").sQuote()); }                
                
                using (SQLDB sql = new SQLDB())
                {   mvDT = sql.GetDT(sSQL.ToString());}

                mvDT.DefaultView.Sort = "[Employee Name]";
