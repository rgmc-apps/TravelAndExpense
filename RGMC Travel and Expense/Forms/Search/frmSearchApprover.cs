...
namespace MyRIS
{
    public partial class frmSearchApprover : Form
    {
        ...
        private const int IX_GRID_EMAIL = 2;

        public frmSearchApprover()
        {
            InitializeComponent();
        }
        ...
        private void GetDetailInfo()
        {
            try
            {
                ...
                mvEmail = dgrid.SelectedRows[0].Cells[IX_GRID_EMAIL].Value.ToString();
        ...
        private void LoadListView()
        {
            ...
            try
            {
                ...
                sSQL.AppendLine("SELECT SU.employeeId AS [ID], SU.name AS [Name], SU.email AS [Email]");
                sSQL.AppendLine("FROM SystemUser SU");           
                sSQL.AppendLine("INNER JOIN SystemMember SM");
                sSQL.AppendLine("   ON SM.secCode = SU.secCode");

                if (mvType != "")
                { sSQL.AppendLine("    AND SM.groupCode = " + mvType.sQuote()); }
                else
                { sSQL.AppendLine("   AND SM.groupCode = 'APPROVER'"); }
