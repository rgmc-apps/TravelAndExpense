...
namespace MyRIS
{
    public partial class frmSearchStores : Form
    {
        ...
        private void LoadListView()
        {
            ...
            try
            {
                ...
                sSQL.AppendLine("SELECT storeId AS [ID], storeName AS [Store]");
                sSQL.AppendLine("FROM Stores");
                //sSQL.AppendLine("WHERE isActive = 1");

                if (strName != "")
                { sSQL.AppendLine("    WHERE storeName LIKE " + ("%" + strName + "%").sQuote()); }

                using (SQLDB sql = new SQLDB())
                { dt = sql.GetDT(sSQL.ToString()); }

                dt.DefaultView.Sort = "[Store]";
        ...
        private void frmSearchStores_Load(object sender, EventArgs e)
        {
            txtName.Text = mvName;
